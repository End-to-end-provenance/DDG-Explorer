...
public abstract class WorkflowParser {
	...
	protected WorkflowGraphBuilder builder;
	 ...
	 * @param WorkflowGraphBuilder the object to build the visual graph
	 * @return the parser that can handle the given file
	 * @throws IOException if there are problems reading the file
	 */
	public static WorkflowParser createParser(File file, WorkflowGraphBuilder workflowGraphBuilder) throws IOException {
		String fileName = file.getName();
		String extension = fileName.substring(fileName.lastIndexOf('.') + 1);
		if (extension.equals("txt")) {
			return new WorkflowTextParser (file, workflowGraphBuilder);
		}
		else if (extension.equals("json")) {
			return new WorkflowJSonParser (file, workflowGraphBuilder);
		}
		else {
