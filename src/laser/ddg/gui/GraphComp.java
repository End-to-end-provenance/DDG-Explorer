...
public class GraphComp extends JPanel {
	...
	// private JButton selectFile1Button = new JButton ("Select R script");
	
	// // The button used to select the left file from the database
	// private JButton selectDdg1Button = new JButton ("Select ddg.txt");
	
	// // The button used to select the right file from the file system
	// private JButton selectFile2Button = new JButton ("Select R script");
	
	// // The button used to select the right file from the database
	// private JButton selectDdg2Button = new JButton ("Select ddg.txt");
	...
	private JTextField leftDdgField = new JTextField();
	
	// The field to display the right file name
	private JTextField rightFileField = new JTextField();

	private JTextField rightDdgField = new JTextField();

	private HashMap<String, String> leftHashSpace = new HashMap<String, String>();

	private HashMap<String, String> rightHashSpace = new HashMap<String, String>();

	private HashMap<String, String> leftHashNum = new HashMap<String, String>();

	private HashMap<String, String> rightHashNum = new HashMap<String, String>();

	
	// references for pop-ups
	private JFrame frame;

	public Parser parserLeft;
	
	public Parser parserRight;

	private Writer writerLeft;

	private Writer writerRight;

	private Writer writerDiff;

	private String diffOutput;

	private String[] diffOutputArray;
	...
	private void displayGraphDiff(){
			...
			JenaWriter jenaWriterRight = JenaWriter.getInstance();
         	...
         	PrefuseGraphBuilder builderLeft = new PrefuseGraphBuilder(false, jenaWriterLeft);
         	PrefuseGraphBuilder builderRight = new PrefuseGraphBuilder(false, jenaWriterRight);
		 	...
		 	builderLeft.compareDDG = true;
		 	builderRight.compareDDG = true;
			...
			try
			{
				writerLeft = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("leftTemp.txt"), StandardCharsets.UTF_8));
				writerRight = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("rightTemp.txt"), StandardCharsets.UTF_8));
			}
			catch(Exception e)
			{
			...
			for(int i=1; i<=parserLeft.numPins;i++){
				...
				leftHashSpace.put(curNodeName.replaceAll("\\s+",""), curNodeName);
				try
				{
					...
					if(dummy.indexOf("[")<0)
					{
						...
						leftHashNum.put(add, curNodeName);
			...
			for(int i=1; i<=parserRight.numPins;i++){
				...
				System.out.println(curNodeName.replaceAll("\\s+",""));
				rightHashSpace.put(curNodeName.replaceAll("\\s+",""), curNodeName);
				try
				{
					String dummy = curNodeName.replaceAll("\\s+","");
					if(dummy.indexOf("[")<0)
					{
						String add = dummy.substring(dummy.indexOf('-')+1);
						rightHashNum.put(curNodeName, add);
				...
				{
					System.out.println("writing to right file error");
				}
			}
    		...
    		diffOutput = com.executeCommand("diff -y -w -b leftTemp.txt rightTemp.txt").trim();
    		System.out.println(diffOutput);	
    		diffOutputArray = diffOutput.split("\n");
			...
			vfListener vfL = new vfListener(display_left, displayOverview_left, display_right, displayOverview_right);
			...
			newPanel_left.addComponentListener(new ComponentAdapter() {

			@Override
			public void componentResized(ComponentEvent e) {
				int panelHeight = newPanel_left.getHeight();
				Rectangle prevBounds = displayOverview_left.getBounds();
				displayOverview_left.setBounds(prevBounds.x, prevBounds.y,
			...
			newPanel_right.addComponentListener(new ComponentAdapter() {
			
			@Override
			public void componentResized(ComponentEvent e) {
				int panelHeight = newPanel_right.getHeight();
				Rectangle prevBounds = displayOverview_right.getBounds();
				displayOverview_right.setBounds(prevBounds.x, prevBounds.y,
						prevBounds.width, panelHeight - 16);
			}
		});
			...
			add(finalContent);
    		diffOutput = com.executeCommand("rm leftTemp.txt rightTemp.txt").trim();
	    ...
	    zoomSetting.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if(currentScale_right != zoomedScale_right){
					...
					int xMiddle_right = (int)(frameBounds_right.getWidth()-frameBounds_right.getX())/2;
					int yMiddle_right = (int)(frameBounds_right.getHeight()-frameBounds_right.getY())/2;
...
class vfListener implements MouseInputListener{
	    ...
	    public vfListener(DDGDisplay userDisplay_left, DDGDisplay overview_left, DDGDisplay userDisplay_right, DDGDisplay overview_right){
	    	super();
