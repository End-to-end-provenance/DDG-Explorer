...
/**
 * A query that allows the user to search for files used within a DDG database.  There are
 * a variety of options that the user can set via the gui:
 *   - input files, output files or both
 *   - file extension
 *   - search entire db, all ddgs for one script, or one ddg
 *   
 * The results are displayed in a table showing file name, node name, script and timestamp.
 * The user can then select a file and operate on it.
 * 
...
public class FileUseQuery extends AbstractQuery {
	...
	// The timestamp of the ddg to search or null if the user is searching entire db
	 ...
	 * @param parent the parent window, used to control where this window pops up
	 ...
	 * @param dbLoader the object that can read from the db
	 * @param selectedIO whether to return input files, output files or both
	 * @param selectedExtensions the extensions limiting which types of files to return.  An empty
	 * 		list means all file extensions
	 */
	private void displayFilenames(JenaLoader dbLoader, int selectedIO, ArrayList<String> selectedExtensions) {
		try {
			...
			// Get files from the db, restricting to input, output or all based on parameter passed in
			switch (selectedIO) {
			case INPUT:
			...
			final String[] columns = { "File name", "Script", "DDG Timestamp",
					"Node name" };
			...
			//final JButton compareFilesButton = new JButton("Compare File(s)");
