 ...
 * 
 */
public class WorkflowLayout extends TreeLayout {
	 ...
	 */
	public WorkflowLayout(String group, boolean dataDerivation) {
		super(group);
...
//		System.out.println("    prelim = " + nodeParams.prelim);
		...
		if (node.isExpanded() && node.isVisible() && (PrefuseUtils.isProcNode(node) || dataDerivation || depth == 0)) { 
			...
			while (unattachedParentIter.hasNext()) {
				...
				secondWalk(nextExternalInput, node, params.mod + parentShift, depth);
			}
